...
<body class="bg-gray-50 font-sans antialiased">
    ...
    <main class="max-w-3xl mx-auto px-6 lg:px-8 pt-28 pb-16">
        <!-- Success Card -->
        <div class="bg-white rounded-3xl border border-gray-100 p-8 md:p-12 text-center mb-8">
            <div class="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6 animate-check">
                <i class="fas fa-check text-3xl text-green-500"></i>
            </div>
            <h1 class="text-2xl md:text-3xl font-extrabold text-gray-900 mb-2">Pembayaran Terkirim!</h1>
            <p class="text-gray-500 mb-1">Bukti pembayaran kamu telah berhasil dikirim.</p>
            <p class="text-sm text-gray-400">Pesanan akan segera diproses oleh admin.</p>

            <div class="mt-8 inline-flex items-center gap-2 bg-orange-50 text-orange-600 font-bold text-lg px-6 py-3 rounded-2xl">
                <i class="fas fa-hashtag"></i> Pesanan #{{ $order->id }}
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 p-6">
                <div class="flex items-center gap-3 mb-5">
                    ...
                    <h2 class="font-bold text-gray-900">Buku Dipesan</h2>
                </div>
                <div class="space-y-3">
                    @foreach ($order->items as $item)
                    <div class="flex gap-3">
                        <div class="w-10 h-14 shrink-0 rounded-lg overflow-hidden bg-orange-50 flex items-center justify-center">
                            @if ($item->book && $item->book->cover)
                            <img src="{{ asset('storage/' . $item->book->cover) }}" alt="" class="w-full h-full object-cover">
                            @else
                            <i class="fas fa-book text-orange-200 text-sm"></i>
                            @endif
                        </div>
                        ...
                        <p class="text-sm font-bold text-orange-500 shrink-0">Rp {{ number_format($item->harga * $item->jumlah, 0, ',', '.') }}</p>
                    </div>
                    @endforeach
                </div>
            ...
            <div class="space-y-6">
                ...
                <!-- Status -->
                <div class="bg-green-50 rounded-2xl border border-green-100 p-5 flex items-start gap-3">
                    <div class="w-9 h-9 bg-green-100 rounded-full flex items-center justify-center shrink-0">
                        <i class="fas fa-check-circle text-green-500 text-sm"></i>
                    </div>
                    <div>
                        <p class="text-sm font-bold text-green-700">Bukti Pembayaran Terkirim</p>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 p-6">
                    ...
                    <img src="{{ asset('storage/' . $order->bukti_pembayaran) }}" alt="Bukti Pembayaran" class="w-full rounded-xl border border-gray-100">
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="text-center">
            <a href="{{ route('user.dashboard') }}" class="inline-flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white font-semibold text-sm px-8 py-3 rounded-full transition-colors shadow-sm shadow-orange-500/30">
